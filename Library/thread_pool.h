...
namespace core 
{
    ...
    */
    struct worker_thread
    {
        ...
        static void prepare_request(work_unit* p_request);
        static DWORD WINAPI thread_proc(LPVOID) throw();
        static void execute(work_unit* p_request) throw();
    }; // struct worker_thread
    ...
    */
    ...
    : m_b_shutdown_flag(false), m_h_iocp(0)
    {
    }
    ...
    //  chain::data
    ///////////////////////////////////////////////////////////////////////////////
    inline void chain::data::process() throw()
    {
        _ASSERT(size());
        _ASSERT(front().get());
        
        if (empty()) return;
        if (0 == front().get()) return;
        front().get()->process();
        pop_front();
    ...
    // worker_thread
    ////////////////////////////////////////////////////////////////////////////////
    inline void worker_thread::prepare_request(work_unit* p_request) 
    { 
        p_request->add_ref(); 
    }
    ...
    inline void worker_thread::execute(work_unit* p_request) throw()
    {
